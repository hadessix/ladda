...
> **⚠️ ห้ามใส่ `<link rel="manifest">` ใน HTML แบบตายตัว** — iOS อ่าน manifest ตอน parse แล้วใช้ `start_url` เป็นลิงก์ไอคอน ทำให้ `#k=` หลุดหาย
> ต้องใส่ด้วย JS เฉพาะเครื่องที่**ไม่ใช่** iOS (ดู `authInit()`) — เรื่องนี้ไล่หาอยู่หลายรอบกว่าจะเจอ
...
| `_devDiag()` | บรรทัดสถานะบนหน้า login (มีกุญแจ/มีบัตร/URL/โหมด) — ไว้ไล่ปัญหา |
