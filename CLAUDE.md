...
| `POST /att/tap` | device token | บันทึกการแตะ + พิกัด |
| `GET /me` | device token **หรือ** `X-View-Key` | ข้อมูลของเจ้าของเท่านั้น (อ่านอย่างเดียว) |
...
- `DUP_WINDOW_SEC = 120` — แตะซ้ำจุดเดิมใน 2 นาที = นับครั้งเดียว
...
1. **กดปุ่ม "⏱ ลงเวลาเข้างาน" ในแอป** — ต้องผ่านสแกนนิ้ว/ใบหน้า (`_bioOk`) ก่อน → บันทึกพร้อม `flags.manual=true`
2. **แตะสติ๊กเกอร์ที่รถ** — บันทึกแบบไม่มี `manual`

`_todayStatus()` / `_statusCard()` แสดงเช็กลิสต์ 2 ข้อในหน้า ID Worker · ครบทั้งคู่ = เขียว
ปุ่มลงเวลาจะซ่อนหลังกดแล้ว เหลือแต่การ์ดเตือนว่ายังต้องไปแตะ
ฝั่ง admin: การ์ดสรุป "ยังไม่แตะสติ๊กเกอร์" + ป้ายเหลืองบนชื่อ + เวลาที่กดในแอปเป็นสีเหลือง 📱แอป
...
- **iOS ไม่ให้ขอตำแหน่งถ้าไม่ได้เกิดจากการกดของผู้ใช้** — หน้าแตะที่ทำงานเองอัตโนมัติจึงมักไม่ได้พิกัดครั้งแรก
  แก้ชั่วคราวด้วยปุ่ม "แตะใหม่พร้อมตำแหน่ง" (การกด = ปลดล็อก)
- `_locErr` แยกสาเหตุ: `denied` (กดปฏิเสธ) vs `timeout` (หาไม่เจอ)
- ไม่มีพิกัดก็ยัง**บันทึกเวลาให้** แค่ติดธง `noloc` — การลงเวลาสำคัญกว่า ไม่บล็อกเพราะ GPS
...
| `attTap(pointId)` | หน้าแตะ: ขอพิกัด → POST /att/tap → โชว์เวลา + ครั้งที่ |
...
| `_todayStatus()` / `_statusCard()` | เช็กลิสต์เข้างานวันนี้ (กดในแอป + แตะสติ๊กเกอร์) |
...
- นับ ครบ/ขาด เทียบ `expected_taps`
- ธง: `noloc` ไม่เปิด GPS · `far` แตะไกลเกินรัศมี · `clockskew` เวลาเครื่องเพี้ยน
- กดที่เวลา → `attEvDetail()` ดูพิกัด + เปิด Google Maps
...
**ตัดสินใจค้าง: หน้าแตะสติ๊กเกอร์จะเอาแบบไหน**
- **ก.** แตะแล้วบันทึกทันที (เร็ว แต่ iOS มักไม่ได้พิกัดครั้งแรก ต้องกดปุ่มเหลืองซ้ำ)
- **ข.** มีปุ่มยืนยัน 1 ที (ได้พิกัดทุกครั้ง + กันแตะโดนโดยไม่ตั้งใจ + เห็นชื่อจุดก่อนยืนยัน) ← แนะนำ
