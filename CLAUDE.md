...
## ระบบเช็คเวลาเข้างาน NFC (attendance)

พนักงานเอามือถือแตะสติ๊กเกอร์ NFC บนรถ → บันทึกเวลา + พิกัด · admin ดูกระดานรายวัน
**ยังไม่ต่อเข้า payroll** (ตั้งใจ — คิดเงินเดือนมือไปก่อนจนกว่าจะพิสูจน์ว่าข้อมูลถูก)

### ไฟล์ SQL (รันตามลำดับ)
`attendance_migration.sql` → `attendance_enroll.sql` → `attendance_multi.sql` → `attendance_viewkey.sql` → `attendance_alerts.sql` → `push_migration.sql`

### ตาราง

```sql
att_points (                     -- 1 แถว = 1 สติ๊กเกอร์
  id, name, kind,                -- kind: 'vehicle' (แปะบนรถ) | 'place' (จุดคงที่ เช่น ลานจอดบูรชัย)
  vehicle_id, route_id, lat, lng, radius_m, status
  -- route_id ต้องผูกให้ครบทุกจุด (ตั้งค่าที่ 📍 จุดแตะ → dropdown "ผูกกับสาย")
  -- ไม่งั้นระบบจับเที่ยววิ่งผิด/ไม่จับให้เลย — ดูหัวข้อ "ไปแทนสายอื่น" ด้านล่าง
)
att_devices (                    -- เครื่องที่ผูกกับพนักงาน · 1 คน = 1 เครื่อง (unique index)
  id, employee_id, label, secret_hash, status,   -- pending | active | revoked
  code, claim_hash, claim_token, expires_at,     -- ระบบรหัสจับคู่ 7 หลัก
  view_hash,                                     -- sha256 ของกุญแจดูข้อมูล (ไม่เก็บตัวจริง)
  bound_by, bound_at, revoked_at, last_seen, ua
)
att_events (                     -- append-only ห้ามแก้/ลบจากฝั่งพนักงาน
  id, employee_id, device_id, point_id, route_id, vehicle_id,
  work_date,                     -- ตัดวันที่ 04:00 เวลาไทย (กะข้ามคืน)
  ts,                            -- เวลา server = ตัวจริง
  client_ts,                     -- เวลาจากเครื่อง ไว้จับคนแก้เวลามือถือ
  seq, lat, lng, accuracy, dist_m, flags, ua, ip
)
att_alerts (                     -- แจ้งเหตุจากหน้างาน (ปุ่ม 🚨 รถเสีย)
  id, employee_id, device_id, route_id, kind, work_date, ts,
  lat, lng, accuracy, note, status,   -- status: open | closed
  closed_at, ua, ip
)
push_subs (                      -- เครื่องที่สมัครรับแจ้งเตือน (endpoint unique)
  id, endpoint, role, label, created_at, last_sent, fail_count
)
app_settings (                   -- ค่าตั้งค่าทั่วไป key/value
  key, value jsonb, updated_at   -- ใช้อยู่: 'alert_roles' = ["admin","hr"]
)
route_shifts (                   -- ตั้งค่าในแอป: เงินเดือน → ⏱ เวลาเข้างาน → ⚙️
  route_id, enabled, start_time, trips,
  trips_json,                    -- [{start:'05:30',end:'09:00'}, ...] เวลาต่อเที่ยว
  expected_taps,                 -- 1 + จำนวนเที่ยว×2
  grace_min, work_days,          -- work_days default '1234567' (ทำงานทุกวัน)
  point_mode,                    -- 'vehicle' | 'place' | 'off' (off = ไม่ต้องตอก เช่น ออฟฟิศ)
  place_point_id, note
)
```
...
|---|---|---|
| `POST /att/enroll` | ไม่ต้อง | เครื่องขอรหัสจับคู่ 7 หลัก (เช่น BC37041) หมดอายุ 15 นาที |
| `GET /att/enroll/status?d=&s=` | secret ของเครื่องนั้น | ถามว่าอนุมัติหรือยัง; อนุมัติแล้วคืน token + view_key **ครั้งเดียว** แล้วล้างจาก DB |
| `POST /att/approve` | admin \| hr | อนุมัติรหัส → ผูกกับพนักงาน → ถอนเครื่องเก่าของคนนั้นทั้งหมด |
| `POST /att/tap` | device token **หรือ** `X-View-Key` | บันทึกการแตะ + พิกัด; `route_id` ของแถว = สายที่ผูกกับ**สติ๊กเกอร์**ที่แตะจริง (ไม่ใช่สายของคนแตะ) ตกไปใช้สายของคนแตะถ้าสติ๊กเกอร์ไม่ผูกสาย/เป็นการกดในแอป |
| `GET /me` | device token **หรือ** `X-View-Key` | ข้อมูลของเจ้าของเท่านั้น (อ่านอย่างเดียว) คืน `shifts{routeId:trips_json}` + `routeNames{routeId:name}` ของ**ทุกสายที่แตะวันนี้** ไม่ใช่แค่สายประจำตัว |
| `POST /att/alert` | device token **หรือ** `X-View-Key` | แจ้งเหตุ (รถเสีย) + เรียก `notifyRoles()` ส่ง push |
| `POST /push/subscribe` `/unsubscribe` | ทุก role ยกเว้น device | สมัคร/ยกเลิกรับแจ้งเตือนของเครื่องนั้น |
| `POST /push/test` | admin | ส่ง push ทดสอบไปที่ endpoint ตัวเอง |

> `/att/tap` รับ `X-View-Key` ด้วย เพราะไอคอนหน้าจอโฮมบน iPhone มีแต่กุญแจ ไม่มี device token

- `AUTO_REVOKE_DAYS = 3` — ไม่แตะเกิน 3 วัน → ถอนเครื่องอัตโนมัติ บังคับไปหาหัวหน้างาน (กันลืมเอาคนออกตอนลาออก); ใช้กับกุญแจดูข้อมูลด้วย
- `DUP_WINDOW_SEC = 20` — แตะซ้ำจุดเดิมใน 20 วิ = นับครั้งเดียว (กันแตะเบิ้ล/NFC อ่านซ้ำ; เดิม 120 วิ ยาวไปจนบล็อกรอบจริงถัดไป)
- `CLOCK_SKEW_SEC = 300` — เวลาเครื่องต่างจาก server เกิน 5 นาที = ตีธง
- `AWAY_WINDOW_MIN = 15` / `AWAY_DIST_M = 800` — ทุกครั้งที่ `/att/tap` เทียบพิกัดกับเพื่อนร่วม**สาย** (ไม่ใช่ route ประจำตัว — ดู "ไปแทนสายอื่น") ที่แตะในช่วง 15 นาทีล่าสุด ถ้าห่างกันเกิน 800 ม. → `flags.away` = ระยะทาง + `flags.away_emp` = id เพื่อนที่เทียบด้วย; **ตั้งใจไม่ส่งธงนี้ไปโชว์ฝั่งพนักงาน** เห็นเฉพาะกระดาน admin (`renderPrAtt`/`attEvDetail`) กันคนแก้พฤติกรรมเมื่อรู้ว่าถูกจับตา
- **device token เข้า `/api/:table` ไม่ได้เด็ดขาด (403)** ทำได้แค่ `/att/tap` กับ `/me`

### กุญแจดูข้อมูล (view key) — แก้ปมที่เก็บข้อมูลแยกของ iOS

**ปัญหา:** iOS ให้ไอคอนหน้าจอโฮมมี localStorage แยกจาก Safari และการแตะ NFC เปิดใน Safari เสมอ
→ ถ้าใช้ localStorage อย่างเดียว ไอคอนจะไม่รู้จักเจ้าของ (และลงทะเบียนซ้ำก็ไม่ได้เพราะกฎ 1 คน = 1 เครื่อง)

**ทางแก้:** ย้ายตัวตนไปไว้ใน URL ของไอคอน
- URL ประจำตัว = `/?me=1#k=<กุญแจ 40 ตัวอักษร>`
- **กุญแจอยู่หลัง `#`** → เบราว์เซอร์ไม่ส่งไปเซิร์ฟเวอร์ ไม่โผล่ใน log; เวลาเรียก API ส่งผ่าน header `X-View-Key`
- หน้าเว็บอ่านจาก `#` แล้วเก็บลง localStorage ของบริบทนั้นเอง (`_grabKey()`)
- DB เก็บแค่ `sha256(กุญแจ)` — ฐานข้อมูลหลุดก็ถอดกลับไม่ได้
- กุญแจ**ดูได้อย่างเดียว แตะเวลาไม่ได้** และดึงได้เฉพาะข้อมูลเจ้าของ

### ไอคอนหน้าจอโฮม (PWA) — กฎที่ห้ามแก้

> **⚠️ ห้ามใส่ `<link rel="manifest">` ใน HTML แบบตายตัว**
> iOS อ่าน manifest ตอน parse แล้วใช้ `start_url` ของ manifest เป็นลิงก์ไอคอน → `#k=<กุญแจ>` หลุดหาย → ไอคอนพนักงานเปิดมาไม่รู้จักใคร
> ต้องใส่ด้วย JS เฉพาะเครื่องที่**ไม่ใช่** iOS (ดู `authInit()`) — เรื่องนี้ไล่หาหลายรอบกว่าจะเจอ

- `apple-mobile-web-app-capable` **ใส่ไว้** → ไอคอนเปิดเต็มจอเหมือนแอปจริงทั้ง iOS/Android
  (ใส่ได้เพราะตัวตนพนักงานอยู่ใน URL ไม่ได้อยู่ใน localStorage แล้ว — ถ้าย้อนกลับไปใช้ localStorage ต้องถอดออก)
- `_setIcon()` สลับ `apple-touch-icon` + ชื่อไอคอน + `document.title` ตามบริบท:
  - แอปงาน → `icon.png` (รูปน้องลัดดา) ชื่อ "น้องลัดดา"
  - หน้าพนักงาน/แตะเวลา → `icon-192.png` (หน้าปัดนาฬิกา) ชื่อ "ลงเวลา"
  - **`?work=1` ชนะเสมอ** แม้เครื่องนั้นจะมีบัตรพนักงานค้างอยู่ (ไม่งั้นเพิ่มไอคอนแอปงานไม่ได้)
- `icon-192.png` / `icon-512.png` สร้างด้วยสคริปต์ (นาฬิกาเขียวบนพื้นกรมท่า) — `icon.png` เดิมเป็น **JPEG 922×1152 ไม่จัตุรัส** ใช้เป็นไอคอน manifest ไม่ได้
- `sw.js` เป็น service worker **แบบส่งผ่านอย่างเดียว ห้าม cache** ไม่งั้นผู้ใช้จะค้างเวอร์ชันเก่าหลัง deploy

### เข้างาน = ต้องครบ 2 อย่าง
1. **กดปุ่ม "⏱ ลงเวลาเข้างาน" ในแอป** — ต้องผ่านสแกนนิ้ว/ใบหน้า (`_bioOk`) ก่อน → `attTap(null,true)` → ตรง `_tapGo` ทันที (ข้ามหน้ายืนยันแตะ เพราะมีการสแกนตัวตนเป็นการยืนยันอยู่แล้ว) → บันทึกพร้อม `flags.manual=true`
2. **แตะสติ๊กเกอร์ที่รถ** — บันทึกแบบไม่มี `manual`

> **เหตุผล:** ถ้าปุ่มในแอปนับเป็นเข้างานเลย ลูกน้องจะไม่เดินไปที่รถ

**หน้าแตะสติ๊กเกอร์ใช้แบบ ข. (มีปุ่มยืนยัน)** — ตัดสินใจแล้ว: `attTap(pointId)` (ไม่ใช่ manual) โชว์หน้า "แตะสติ๊กเกอร์แล้ว — กดยืนยันเพื่อบันทึกเวลา" ก่อนเสมอ ต้องกดปุ่ม "ยืนยันลงเวลา" ถึงเรียก `_tapGo()` จริง
- **เหตุผล:** iOS ไม่ยอมขอตำแหน่งถ้าไม่ได้เกิดจากการกดของผู้ใช้ตรงๆ — หน้าแตะอัตโนมัติ (ไม่มีปุ่มกด) มักไม่ได้พิกัดครั้งแรก ต้องมีปุ่มให้กดก่อนเสมอ
- `_locPerm()`: เช็คสถานะสิทธิ์ตำแหน่งแบบเงียบๆ ผ่าน `navigator.permissions` (เบราว์เซอร์ไม่รองรับ เช่น iOS Safari รุ่นเก่า → คืน `null` ข้ามไปเงียบๆ) ถ้าเจอว่าเคย `denied` โชว์คำเตือนบนหน้ายืนยันเลย ไม่ต้องรอกดแล้วพังก่อนถึงจะรู้สาเหตุ — ยังกดยืนยันได้ปกติ แค่จะไม่มีพิกัดแนบไป
- `_tapNoDev()`: หน้า "ยังไม่ได้ลงทะเบียน" (ไม่มีบัตร/กุญแจเลย) — แยกจาก `attTap` ตั้งแต่มีหน้ายืนยันมาคั่นกลาง

**สถานะวันนี้ (`_todayStatus()` / `_statusCard()`)** — ตารางเช็คลิสต์ ไม่ใช่แค่ 2 บรรทัดเฉยๆ:
```
เข้างานวันนี้           [✓][✓]      ← กดในแอป + แตะสติ๊กเกอร์ (ครั้งแรกของแต่ละแบบ)

เที่ยวที่ 1   ขาไป [✓]   ขากลับ [✓]
เที่ยวที่ 2   ขาไป [ ]   ขากลับ [ ]

🔄 ไปแทนสาย จอมทอง        ← โผล่เองถ้าแตะสติ๊กเกอร์สายอื่น
เที่ยวที่ 2   ขาไป [✓]   ขากลับ [ ]
```
- แตะสติ๊กเกอร์**ครั้งแรกของวัน**นับเป็นทั้ง "เข้างาน" และ "เที่ยวที่ 1 ขาไป" พร้อมกัน (ในความเป็นจริงคือแตะครั้งเดียวตอนขึ้นรถ ไม่ใช่แตะ 2 รอบแยกกัน)
- ปุ่มลงเวลาจะซ่อนหลังกดแล้ว เหลือแต่การ์ดเตือนว่ายังต้องไปแตะ

#### ไปแทนสายอื่น (substitute coverage)
ปัญหาที่แก้: การแตะเดิมผูกกับ **สายประจำตัวคนแตะ** เสมอ ถ้าสีทอง (สายตากสิน) ไปแตะแทนจอมทองตอนรถจอมทองเสีย ระบบเดิมจะจับว่าเป็นสายตากสินผิดๆ

**ทางแก้:** `route_id` ของการแตะแต่ละครั้ง = สายที่ผูกกับ**สติ๊กเกอร์**ที่แตะจริง (`point.route_id`) ไม่ใช่สายของคนแตะ (Worker `/att/tap`) แล้ว `/me` คืนตารางเที่ยว+ชื่อของ**ทุกสาย**ที่แตะวันนั้น ไม่ใช่แค่สายประจำตัว

**`_assignTrips(taps,cfg)`** — จับคู่การแตะแต่ละครั้งกับ "เที่ยว" ตาม**เวลาที่ใกล้เคียงที่สุด** (เทียบกับ `start` สำหรับขาไป / `end` สำหรับขากลับ) **ไม่ใช่ตามลำดับที่แตะ**
- เหตุผลที่ไม่ใช้ลำดับ: ถ้าไปแทนแค่เที่ยวเดียว (เที่ยวอื่นของสายนั้นมีคนอื่นทำไปแล้ว/ยังไม่ทำ) การนับตามลำดับการแตะของ**ตัวเอง**จะเพี้ยน เช่น แตะแทนแค่ "เที่ยว 2 ขาไป" ครั้งเดียว ถ้านับลำดับจะกลายเป็น "เที่ยว 1 ขาไป" ผิด ต้องเทียบเวลาแทนถึงจะถูกสาย/ถูกเที่ยว
- `_todayStatus()` แยกการแตะเป็นกลุ่มตาม `route_id` ของแต่ละครั้ง → สายตัวเองโชว์เสมอ (แม้ยังไม่แตะ) ส่วนสายที่ไปแทนโชว์เฉพาะเที่ยวที่แตะจริง (กรองเที่ยวว่างออก)
- ต้องผูก `route_id` ให้ครบทุกจุดแตะก่อน (📍 จุดแตะ → dropdown) ไม่งั้นจับกลุ่ม/จับเที่ยวไม่ได้
- ถ้าแตะสายที่ `_meData.shifts` ยังไม่มีข้อมูล (เพิ่งไปแทนสายใหม่ครั้งแรกวันนั้น) `_tapGo` จะดึง `/me` ใหม่ทั้งชุดแทนอัปเดตในเครื่อง (กันข้อมูลไม่ครบ + ตั้งค่า `refetched` กันไม่ให้ push event ซ้ำ)

**กระดานฝั่ง admin (`renderPrAtt`)** — รวมคู่ "เข้างาน" (แอป+สติ๊กเกอร์ ครั้งแรกของแต่ละแบบ) เป็นแท็กเดียว ไม่ใช่โชว์แยก 2 เวลา:
- `_attPairRest(list)` แยก "คู่เข้างาน" ออกจากรายการที่เหลือ (ออก/กลับแต่ละเที่ยว ยังโชว์แยกเป็นชิปเวลาเหมือนเดิม)
- ครบคู่ → แท็กเขียว "✓ เข้างาน HH:MM" กด → `_attPairDetail()` เห็นทั้ง 2 รายการ ลบได้ทีละอัน (`_attPairEvDelete`)
- ยังไม่ครบ → แท็กเหลือง "⚠ เข้างาน HH:MM"
- ฝั่งขวาเปลี่ยนจาก "X/Y ครั้ง" (นับรวมเข้างาน+เที่ยวปนกัน งง) เป็น **"เที่ยว X/Y"** นับเฉพาะรอบออก-กลับจริง
- `attEvDetail()` มีปุ่ม **"ลบรายการนี้"** (admin, `_attEvDelete`) — ลบรายการแตะทีละอัน ใช้ล้างข้อมูลทดสอบเองได้ไม่ต้องรอแก้ที่ Supabase
- `_expTaps(sh)` = `2 + เที่ยว×2` (เข้างาน 2 + ออก-กลับเที่ยวละ 2) — เดิมเป็น `1+เที่ยว×2` นับผิดเพราะตอนนั้นเข้างานยังไม่ต้อง 2 factor

**หน้า ID Worker ส่วน "7 วันล่าสุด"** ก็รวมคู่เข้างานเป็นก้อนเดียวเหมือนกัน (ใช้ logic คล้าย `_attPairRest` แบบง่าย ไม่แยกหลายสาย — ยังเป็น scope วันเดียวไม่ย้อนหลังหลายสาย)

### ปลดล็อกข้อมูลส่วนตัว (WebAuthn)
`_bioOk()` เรียก Face ID / ลายนิ้วมือของเครื่อง — เว็บไม่เห็นรหัส ได้แค่ผ่าน/ไม่ผ่าน
- ไม่รองรับ / ตั้งค่าไม่ได้ → **ให้ผ่าน** (ไม่ล็อกคนออกจากข้อมูลตัวเอง)
- ผู้ใช้กดยกเลิกหรือสแกนไม่ผ่าน (`NotAllowedError`/`AbortError`) → ไม่ให้เข้า
- ใช้กับ: หน้าส่วนตัว (`_meSecure` → วันหมดอายุ + สลิป) และปุ่มลงเวลา (`_checkInGo`)

### แจ้งเหตุ + แจ้งเตือน (Web Push)
- ปุ่ม 🚨 กลมเล็กจางๆ ใต้ปุ่ม 🔒 ในหน้า ID Worker (เจตนาให้กดยาก กันมือลั่น) → หน้ายืนยันในแอป → `_sosGo()`
- ส่งพิกัดเข้าระบบ แล้วมีปุ่มแชร์ต่อเข้า **LINE/แอปอื่น** (`navigator.share`) และ **Messenger** (`fb-messenger://share`)
- admin เห็นแถบแดงบนกระดานลงเวลา + ปุ่มเปิด Google Maps + ปุ่ม "เสร็จแล้ว" (`_attAlertClose`)
- **Web Push**: VAPID secrets `VAPID_PUBLIC` / `VAPID_JWK` / `VAPID_SUBJECT`
  ส่งแบบ **ไม่มี payload** → ไม่ต้องเข้ารหัส RFC8291; `sw.js` ปลุกขึ้นมาแล้วแสดงข้อความเอง
- ปุ่ม 🚨 ในกระดานลงเวลา (`openAlertSettings`) — เปิด/ปิดรับแจ้งเตือนของเครื่องนี้ + ติ๊กว่า**ตำแหน่งไหน**ได้รับ
  (เก็บใน `app_settings.alert_roles`) → เพิ่มตำแหน่งใหม่ในอนาคตไม่ต้องแก้โค้ด
- ต้องผ่าน **2 เงื่อนไข**: เครื่องนั้นกดเปิดรับ **และ** ตำแหน่งนั้นถูกติ๊กไว้

> **iPhone แจ้งเตือนได้เฉพาะตอนเปิดจากไอคอนหน้าจอโฮม** (Apple บังคับ)
> **service worker ต้องลงทะเบียนทุกเครื่องรวม iOS** — เคยพลาดตรงนี้แล้วปุ่ม 🚨 กดไม่ได้บน iPhone

### แปลภาษา (ไทย / ລາວ / မြန်မာ)
- `_i18nApply()` เดิน text node ใน `#att-screen` หลัง render แล้วแทนข้อความไทย — **ไม่ต้องแก้โค้ดหน้าจอ**
- `I18N` (ตรงตัว) + `I18N_RE` (ข้อความมีตัวเลข) · ตัดอิโมจิหัวท้ายก่อนจับคู่
- ข้าม text node ใน `#lang-bar` (ชื่อภาษาต้องเป็นภาษาตัวเองเสมอ)
- ภาษาเก็บใน localStorage `ladda_lang` · **หน้าแอปงานไม่แปล**
- **ห้ามใช้ `confirm()`/`alert()` ในหน้าพนักงาน** — กล่องของเบราว์เซอร์แปลไม่ได้ ให้ทำหน้ายืนยันในแอปแทน

### ตำแหน่ง (GPS)
`_getLoc()`: แม่นยำสูง 12 วิ → ถ้าไม่ทันใช้เสา/WiFi อีก 8 วิ (`maximumAge:300000`)
- **iOS ไม่ให้ขอตำแหน่งถ้าไม่ได้เกิดจากการกดของผู้ใช้** — แก้ด้วยหน้ายืนยัน "แบบ ข." ก่อน `_tapGo` เสมอ (ปุ่มกด = ปลดล็อกให้ขอพิกัดได้) + ปุ่ม "แตะใหม่พร้อมตำแหน่ง" สำรองถ้ายังพลาด
- `_locErr` แยกสาเหตุ: `denied` (กดปฏิเสธ) vs `timeout` (หาไม่เจอ)
- `_locPerm()` เช็คสิทธิ์ล่วงหน้าแบบเงียบๆ ก่อนโชว์หน้ายืนยัน — เตือนทันทีถ้าเคยถูกปฏิเสธไว้ (ไม่ต้องรอแตะพังก่อน)
- ไม่มีพิกัดก็ยัง**บันทึกเวลาให้** แค่ติดธง `noloc` — การลงเวลาสำคัญกว่า ไม่บล็อกเพราะ GPS (**ตั้งใจ ไม่บังคับ** — คุยกันแล้วว่าอินเทอร์เน็ต/GPS อ่อนไม่ควรทำให้เข้างานไม่ได้)

### เบราว์เซอร์ในแอปแชท
`_inApp()` ตรวจ LINE/Messenger/Facebook/Instagram — แอปพวกนี้มี localStorage แยกจาก Safari/Chrome
เปิดลิงก์แตะจากในนั้นจะไม่เจอบัตร → หน้า "ยังไม่ได้ลงทะเบียน" จะบอกให้กด ⋯ → เปิดในเบราว์เซอร์

### URL ทั้งหมด

| URL | ใครใช้ |
|---|---|
| `/?tap=<point_id>` | อยู่ในสติ๊กเกอร์ NFC — **ห้ามบันทึกลงหน้าจอโฮม** |
| `/?enroll=1` | อยู่ใน QR ที่หัวหน้าเปิดให้สแกน |
| `/?me=1#k=<กุญแจ>` | ลิงก์ประจำตัวพนักงาน — อันนี้ที่บันทึกลงหน้าจอโฮม |
| `/?work=1` | บังคับเข้าแอปงาน (เครื่องที่มีบัตรพนักงานค้างอยู่) |
| `/` | แอปงานปกติ |

### ฟังก์ชันฝั่งหน้าเว็บ

| ฟังก์ชัน | หน้าที่ |
|---|---|
| `attRoute()` | router เรียกจาก `authInit()` — ตัดสินว่าจะเข้าหน้าไหน |
| `attTap(pointId,manual)` | `manual=true` (จากปุ่มแอป) → ตรง `_tapGo` เลย; ไม่งั้นโชว์หน้ายืนยันก่อน (แบบ ข.) |
| `_tapGo(pointId,manual)` | ขอพิกัด → POST /att/tap → โชว์เวลา + ครั้งที่ + การ์ดสถานะ |
| `_tapNoDev()` | หน้า "ยังไม่ได้ลงทะเบียน" (ไม่มีบัตร/กุญแจ) |
| `_locPerm()` | เช็คสถานะสิทธิ์ตำแหน่งแบบเงียบๆ (`navigator.permissions`) ไม่รองรับ → `null` |
| `_assignTrips(taps,cfg)` | จับคู่การแตะกับเที่ยวตามเวลาใกล้เคียงที่สุด (ไม่ใช่ตามลำดับ) — รองรับไปแทนสายอื่น |
| `_attPairRest(list)` / `_attTapChip(v)` | (กระดาน admin) แยกคู่เข้างานออกจากรายการที่เหลือ / render ชิปเวลา 1 อัน |
| `_attPairDetail(id1,id2)` / `_attPairEvDelete` | โมดัลดูคู่เข้างาน 2 รายการ + ลบทีละอัน |
| `_attEvDelete(evId)` | (admin) ลบรายการแตะ 1 รายการ — ใช้ล้างข้อมูลทดสอบ |
| `attEnroll(force)` | ขอรหัสจับคู่ + poll ทุก 3 วินาที; เครื่องที่มีบัตรแล้วจะไม่ขอซ้ำ |
| `attMe()` | หน้าข้อมูลของฉัน: แจ้งเตือนวันหมดอายุ (60/30 วัน), รูปเอกสาร, สลิปงวดล่าสุด, ประวัติ 7 วัน |
| `_grabKey()` / `_viewKey()` / `_meUrl()` | อ่าน/เก็บกุญแจ · เขียน URL ประจำตัว |
| `renderPrAtt()` | กระดานลงเวลารายวัน (tab ⏱ เวลาเข้างาน) |
| `renderPrShifts()` | หน้าตั้งค่าเวลารายเที่ยว (ปุ่ม ⚙️) |
| `openAttDevices()` | จัดการเครื่อง: QR ลงทะเบียน, รออนุมัติ, ลงทะเบียนแล้ว, ถอน |
| `openAttPoints()` | จัดการสติ๊กเกอร์ + คัดลอกลิงก์ไปเขียนลงสติ๊กเกอร์; จุดที่ id เดาง่าย (ตั้งเองสมัยก่อน เช่น `p_burachai`) โชว์ปุ่ม 🔀 สุ่มรหัสใหม่ |
| `_ptRegen(id)` | สุ่ม id จุดแตะใหม่ (ลบแถวเก่า+สร้างแถวใหม่ข้อมูลเดิม) — ต้องเขียนสติ๊กเกอร์ใหม่ทับหลังกด เพราะลิงก์เดิมใช้ไม่ได้แล้ว |
| `_devApprove(devId,code)` | อนุมัติรหัสให้พนักงานที่เลือก |
| `_devDiag()` | บรรทัดสถานะบนหน้า login (มีกุญแจ/มีบัตร/URL/โหมด) — ไว้ไล่ปัญหาเครื่องลูกน้อง |
| `_checkIn()` / `_checkInGo()` | ปุ่มลงเวลาในแอป → ยืนยัน → สแกนตัวตน → `attTap(null,true)` |
| `_todayStatus()` / `_statusCard()` / `_statusChk()` | สถานะวันนี้แยกตามสาย (สายตัวเอง + ไปแทนสาย) ใช้ `_assignTrips` จับคู่เที่ยว |
| `_bioOk()` | ยืนยันตัวตนด้วย Face ID/ลายนิ้วมือ (WebAuthn) |
| `_sosAsk()` / `_sosGo()` | แจ้งรถเสีย — หน้ายืนยัน แล้วส่งพิกัดเข้าระบบ |
| `_sosShare()` / `_sosMsgr()` | แชร์พิกัดต่อเข้า LINE/แอปอื่น / Messenger |
| `openAlertSettings()` | ปุ่ม 🚨 — เปิด/ปิดรับแจ้งเตือนเครื่องนี้ + เลือกตำแหน่งที่ได้รับ |
| `_pushOn/_pushOff/_pushTest` | จัดการการรับแจ้งเตือนของเครื่องนี้ |
| `_attAlertBar()` / `_attAlertClose()` | แถบแจ้งเหตุบนกระดานลงเวลา / ปิดการแจ้ง |
| `_i18nApply()` / `setLang()` / `_langBar()` | ระบบแปลภาษาหน้าพนักงาน |
| `_inApp()` | ตรวจว่าเปิดอยู่ในเบราว์เซอร์ของ LINE/Messenger หรือไม่ |
| `_setIcon()` | สลับไอคอน/ชื่อหน้าจอโฮมตามบริบท (แอปงาน vs ลงเวลา) |
| `_isStandalone()` / `_isIOS()` | ตรวจว่าเปิดจากไอคอนหน้าจอโฮม / เป็น iPhone หรือไม่ |

### กระดานลงเวลา (`renderPrAtt`)
- เรียงคนตามเวลามาถึงในแต่ละสาย + เลขลำดับ → เห็นว่าใครมาก่อนหลัง
- คำนวณสายกี่นาทีเทียบ `trips_json[0].start` + `grace_min`
- นับ "เที่ยว X/Y" เทียบเฉพาะรอบออก-กลับ (ไม่รวมคู่เข้างานที่ถูกรวมเป็นแท็กแล้ว)
- ธง: `noloc` ไม่เปิด GPS · `far` แตะไกลเกินรัศมี · `clockskew` เวลาเครื่องเพี้ยน
- กดที่เวลา/แท็กเข้างาน → `attEvDetail()` / `_attPairDetail()` ดูพิกัด + เปิด Google Maps + ลบได้

### สติ๊กเกอร์ NFC (ของจริง)
- ใช้ NTAG215 ใบละ ~8 บาท พอ — **สติ๊กเกอร์ไม่ใช่ตัวกันโกง** (ก๊อปได้ และ bookmark URL ได้อยู่ดี)
  ตัวกันโกงจริงคือ **พิกัด + เทียบกับเพื่อนร่วมรถ**
- แปะที่รถ 1 คัน = 1 ใบ · **ห้ามแปะบนโลหะ** (อ่านไม่ขึ้น) — แปะพลาสติก/กระจก หรือใช้รุ่น anti-metal
- เขียนลิงก์ลงสติ๊กเกอร์ด้วยแอป NFC Tools (ฟรี) → Write → URL

### ค้างไว้ — คุยต่อครั้งหน้า

**ตรวจอัตโนมัติ** — 210 แถว/วัน ตรวจมือไม่ไหว ต้องให้ระบบคัดมาให้เหลือวันละ 0-3 บรรทัด
1. [x] ห่างจากเพื่อนร่วมรถเกินกำหนด (แตะเวลาใกล้กันแต่พิกัดห่างกันมาก) ← แรงที่สุด — ทำแล้ว ดู `flags.away`/`AWAY_DIST_M` ด้านบน
2. [ ] พิกัดนิ่งผิดปกติ (แตะทุกครั้งจากจุดเดิมเป๊ะ = น่าจะกดจากบ้าน)
3. [ ] ไม่เคยแตะสติ๊กเกอร์เลย กดปุ่มในแอปอย่างเดียว

**เนื้อที่ฐานข้อมูล** — 42 คน × 5 ครั้ง/วัน ≈ 38 MB/ปี จาก 500 MB ที่มี = ใช้ได้ ~12 ปี ไม่ต้องห่วง
(ถ้าอยากประหยัด ตัดการเก็บ `ua` ในทุก event ออกได้ ลด ~40%)

### ที่ยังไม่ได้ทำ (อื่นๆ)
- [ ] สรุปวันทำงานรายงวด → ต่อเข้า payroll (รอให้มั่นใจก่อน)
- [ ] ให้ลูกน้องเห็นสถิติตัวเองว่าเดือนนี้มาสายกี่ครั้ง
- [ ] admin กรอกเวลาเข้างานให้ครบทุกสาย (งานกรอกข้อมูล ไม่ใช่งานโค้ด)
- [ ] เขียนลิงก์ลงสติ๊กเกอร์ NFC แล้วแปะรถ (ยังไม่ได้ทำเลย)
...
**2. ระบบเช็คเวลาเข้างาน (NFC)** — ✅ ใช้งานได้แล้ว (ดูหัวข้อด้านบน)
- เหลือ: เทียบพิกัดเพื่อนร่วมรถ + ต่อเข้า payroll
